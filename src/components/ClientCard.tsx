...
function ClientCard({
    ...
    return (
        <div
            className={`w-full flex flex-col md:flex-row items-center justify-between gap-10 pt-10 ${reverse ? "md:flex-row-reverse" : ""
                }`}
        >

            {/* TEXT SIDE */}
            <div className="md:w-1/2 text-center md:text-left px-4 md:px-8 mx-6">
                <h1 className="text-3xl sm:text-5xl font-bold leading-tight mt-2 text-slate-100">
                    {title} <br />
                    <span className="text-blue-600">{subtitle}</span>
                </h1>

                <p className="text-gray-400 text-sm sm:text-lg md:text-xl mt-4">
                    {description}
                </p>
            </div>

            {/* IMAGE SIDE */}
            <div className="md:w-1/2 relative group cursor-pointer aspect-[16/9] w-full mx-6">
                <img
                    src={imgSrc}
                    alt={title}
                    loading="lazy"
                ...
                >
                    <span className="bg-blue-600 text-white px-8 py-3 rounded-full text-lg font-semibold tracking-wide shadow-lg hover:scale-110 transition">
                        Visit Site ↗
                    </span>
                </a>
            </div>
